...
if [ -z "$VAULT_TOKEN" ]; then
  echo "VAULT_TOKEN is required to bootstrap Vault." >&2
  exit 1
fi
...
if ! vault read "${VAULT_GENESIS_TRANSIT_MOUNT}/keys/${VAULT_GENESIS_TRANSIT_KEY}" >/dev/null 2>&1; then
...
existing_seed="$(vault kv get -field=opaque_seed_b64 \
  "${VAULT_KV_MOUNT}/${VAULT_OPAQUE_SECRET_PATH}" 2>/dev/null || true)"
seed="${VAULT_OPAQUE_SEED_B64:-$existing_seed}"
if [ -z "$seed" ]; then
  seed=$(head -c 32 /dev/urandom | base64 | tr -d '\n')
fi
if [ -z "$existing_seed" ] || [ "$seed" != "$existing_seed" ]; then
  vault kv put "${VAULT_KV_MOUNT}/${VAULT_OPAQUE_SECRET_PATH}" \
    opaque_seed_b64="$seed" >/dev/null
fi
...
LOGIN_URL="${VAULT_ADDR%/}/v1/auth/${VAULT_APPROLE_MOUNT}/login"

cat <<EOF
Vault server ready on ${VAULT_LISTEN_ADDRESS}
Login URL: ${LOGIN_URL}

...
use permesi::cli::globals::GlobalArgs;
use permesi::vault::{VaultTarget, VaultTransport};
...
async fn mock_agent_service(
    ...
    if method == Method::GET && path == "/v1/database/creds/permesi" {
        let body = json!({
            "lease_id": "lease-socket-permesi-123",
            "lease_duration": 300,
            "data": {
                "username": "socket-user-permesi",
                "password": "socket-password-permesi"
            }
        });
...
async fn test_database_creds_via_socket() -> anyhow::Result<()> {
    ...
    // 4. Call database_creds
    permesi::vault::database::database_creds(&mut globals).await?;

    // 5. Verify results

    assert_eq!(globals.vault_db_lease_id, "lease-socket-permesi-123");

    assert_eq!(globals.vault_db_username, "socket-user-permesi");

    assert_eq!(
        globals.vault_db_password.expose_secret(),
        "socket-password-permesi"
    );

...
//! 4) Expose helpers for outbound HTTPS clients that must trust the same CA.
//!
...
mod tests {
    ...
    async fn test_load_valid_cert_config() {
        ...
        let dir = std::env::temp_dir().join(format!("permesi-tls-test-valid-{}", Uuid::new_v4()));
    ...
    async fn test_watcher_reloads_on_change() {
        let _ = rustls::crypto::aws_lc_rs::default_provider().install_default();
        let dir = std::env::temp_dir().join(format!("permesi-tls-test-watcher-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let cert_path = dir.join("tls.crt");
        let key_path = dir.join("tls.key");
        let ca_path = dir.join("ca.pem");
        ...
        std::fs::write(&key_path, cert_a.signing_key.serialize_pem()).unwrap();
        ...
        // Update certs (Cert B)
        // Ensure mtime changes by sleeping slightly if FS resolution is low,
        // or just rely on the test execution time + 100ms interval.
        ...
        std::fs::write(&key_path, cert_b.signing_key.serialize_pem()).unwrap();
